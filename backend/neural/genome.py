...
class Genome:
    ...
    def mutate_weights(self, perturb_chance=0.9, step_size=0.1):
        for connection in self.connections.values():
            if random.random() < perturb_chance:
                connection.weight += random.uniform(-step_size, step_size)
            else:
    ...
    def add_connection(self, max_attempts=50):
        ...
        for _ in range(max_attempts):
            ...
            existing_connection = any(
                connection.in_node == a and connection.out_node == b
                for connection in self.connections.values()
            )
    ...
    def add_node(self):
        enabled_connections = [connection for connection in self.connections.values() if connection.enabled]
    ...
    def crossover(self, other: 'Genome') -> 'Genome':
        ...
        child.nodes = {node_id: deepcopy(node) for node_id, node in parent1.nodes.items()}
        child.input_ids = list(parent1.input_ids)
        child.output_ids = list(parent1.output_ids)
        child.bias_id = parent1.bias_id

        for innovation_number, connection1 in parent1.connections.items():
            connection2 = parent2.connections.get(innovation_number)
            if connection2 and random.random() < 0.5 and connection2.enabled:
                chosen_connection = connection2
            else:
                chosen_connection = connection1

            child.connections[innovation_number] = ConnectionGene(
                innovation_number=chosen_connection.innovation_number,
                in_node=chosen_connection.in_node,
                out_node=chosen_connection.out_node,
                weight=chosen_connection.weight,
                enabled=chosen_connection.enabled,
                recurrent=chosen_connection.recurrent,
            )

        return child

    def _copy_from(self, other: 'Genome') -> 'Genome':
        self.nodes = {node_id: deepcopy(node) for node_id, node in other.nodes.items()}
        self.connections = {
            innovation_number: deepcopy(connection)
            for innovation_number, connection in other.connections.items()
        }
        self.input_ids = list(other.input_ids)
        self.output_ids = list(other.output_ids)
        self.bias_id = other.bias_id
        self.fitness = 0.0
        return self

    def compatibility_distance(self, other: 'Genome', c1=1.0, c2=1.0, c3=0.4) -> float:
        this_connections = self.connections
        other_connections = other.connections
        max_innovation = max(
            max(this_connections.keys()) if this_connections else 0,
            max(other_connections.keys()) if other_connections else 0,
        )

        disjoint = 0
        weights = []
        for innovation_number in range(1, max_innovation + 1):
            connection1 = this_connections.get(innovation_number)
            connection2 = other_connections.get(innovation_number)
            if connection1 and connection2:
                weights.append(abs(connection1.weight - connection2.weight))
            elif connection1 or connection2:
                disjoint += 1

        average_weight_diff = sum(weights) / len(weights) if weights else 0.0
        N = max(len(this_connections), len(other_connections))
        if N < 20:
            N = 1

        return (c1 * disjoint / N) + (c3 * average_weight_diff)